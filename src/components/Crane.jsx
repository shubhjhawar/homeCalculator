...
import { addCraneItems, changeCraneItemQuantity, removeCraneItem} from '../slices/slices';
...
const Crane = () => {
    ...
    const craneItem = useSelector(state => state.items.craneItems)
    const [toggle, setToggle] = useState(false)
    const [listToggle, setListToggle] = useState(false)
    const [craneItems, setCraneItems] = useState(craneItem)
    const dispatch = useDispatch()
    
    const handleItemSelect = index => {
        const selectedItem = items[index];
        setCraneItems(prevSelectedItems =>
    ...
    const handleQuantityChange = (name, change) => {
        ...
        dispatch(changeCraneItemQuantity({ name, change }));
    ...
    useEffect(() => {
        dispatch(addCraneItems(craneItems))
    }, [craneItems, dispatch])
            ...
            <div className="flex flex-col justify-end w-full">
                ...
                <div className="max-h-[150px] overflow-y-auto">
                    {items.map((item, index) => {
                        const isSelected = craneItems.some(selectedItem => selectedItem.name === item.name);
                        return(
