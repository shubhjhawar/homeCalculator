...
const Assemble = () => {
    ...
    const [assembleItems, setAssembleItems] = useState(assembledItems)
    const dispatch = useDispatch()
    
    const handleItemSelect = index => {
        const selectedItem = items[index];
        setAssembleItems(prevSelectedItems =>
    ...
    const handleSelectAll = () => {
        setAssembleItems(items);
    };

    const handleQuantityChange = (name, change) => {
    
        setAssembleItems(prevItems => {
    ...
    useEffect(() => {
        dispatch(addAssembledItems(assembleItems))
    }, [assembleItems, dispatch])
